...
const chartCellHtml = (chart, index, backgroundColor) => `
...
class ChartGridController {
  ...
  updateCells(layouts, axes, fullscreen) {
    layouts.forEach(({ chart, node }) => {
      ...
      Object.assign(grid.style, {
        left: `${PLOT_PADDING.left}px`, right: `${PLOT_PADDING.right}px`, top: `${PLOT_PADDING.top}px`, bottom: `${PLOT_PADDING.bottom}px`,
      ...
      yAxis.innerHTML = (axis?.ticks || []).map((tick) => `<div class="pointer-events-none absolute left-1/2 -translate-x-1/2 -translate-y-1/2 text-center text-[11px] font-medium tabular-nums text-foreground/70" style="top:${tick.top}px">${escapeHtml(this.options.formatYValue(tick.value))}</div>`).join("") +
  ...
  handleKeyDown(event) {
    ...
    else if (key === "m") { this.toggleMovingAverage(chartId); event.preventDefault(); }
    else if (["t", "h", "v", "p"].includes(key)) { this.toggleDrawingTool({ t: "trendline", h: "hline", v: "vline", p: "pin" }[key]); event.preventDefault(); }
